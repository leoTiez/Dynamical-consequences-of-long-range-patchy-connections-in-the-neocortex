...
def main_error(
    ...
    for file_full in sorted(os.listdir(curr_dir + path_full)):
        ...
        is_attribute = attribute in file_full and attribute in file_part
        ...
        is_stimulus = stimulus in file_full and stimulus in file_part
    ...
    if plot_type == PLOT_TYPE["bar"]:
        ...
        if not save_plot:
            ...
            curr_dir = os.getcwd()
            save_name = curr_dir + "/figures/lost-information/%s_network_%s_80.png" % (network, attribute)
        ...
        if not save_plot:
            ...
            curr_dir = os.getcwd()
            save_name = curr_dir + "/figures/lost-information/gauss_dist_%s_network_%s_80.png" % (network, attribute)
            plt.savefig(save_name)
            plt.close()
...
if __name__ == '__main__':
    ...
    for network in NETWORK_TYPE.keys():
        for tuning in TUNING_FUNCTION.keys():
            main_error(
                path_full="experiments/error-full/error",
                path_part="experiments/error-80-net/error",
                network=network,
